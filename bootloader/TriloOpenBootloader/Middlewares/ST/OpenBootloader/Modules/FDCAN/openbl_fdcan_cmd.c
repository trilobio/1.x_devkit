...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static uint8_t a_OPENBL_FDCAN_CommandsList[OPENBL_FDCAN_COMMANDS_NB_MAX] =
		{ 0U };
static uint8_t FdcanCommandsNumber = 0U;

/* Private function prototypes -----------------------------------------------*/
static uint8_t OPENBL_FDCAN_GetAddress(uint32_t *Address);
static uint8_t OPENBL_FDCAN_GetSpecialCmdOpCode(uint16_t *OpCode,
		OPENBL_SpecialCmdTypeTypeDef CmdType);
static uint8_t OPENBL_FDCAN_ConstructCommandsTable(
		OPENBL_CommandsTypeDef *pFdcanCmd);
...
OPENBL_CommandsTypeDef* OPENBL_FDCAN_GetCommandsList(void) {
	static OPENBL_CommandsTypeDef OPENBL_FDCAN_Commands = {
			OPENBL_FDCAN_GetCommand, OPENBL_FDCAN_GetVersion,
			OPENBL_FDCAN_GetID, OPENBL_FDCAN_ReadMemory,
			OPENBL_FDCAN_WriteMemory, OPENBL_FDCAN_Go,
			...
			NULL,
			NULL,
			NULL, OPENBL_FDCAN_SpecialCommand,
		...
		if (OPENBL_FDCAN_GetAddress(&address) == NACK_BYTE) {
			...
			while (count != 0U) {
				...
				for (counter = 64U; counter > 0U; counter--) {
					TxData[data_length] = OPENBL_MEM_Read(address,
							memory_index);
	...
	if (Common_GetProtectionStatus() != RESET) {
		...
		if (OPENBL_FDCAN_GetAddress(&address) == NACK_BYTE) {
			...
			if (status == 0U) {
				...
				// Datalength code for 16 bytes (12 for UUID 4 for address)
				if (FDCanDataLength == 10) {
					uint32_t uid[3];
					...
					uid[2] = HAL_GetUIDw2();
					uint32_t uid_msg[3];
					uid_msg[0] = (RxData[4] << 24) | (RxData[5] << 16)
							| (RxData[6] << 8) | RxData[7];
					uid_msg[1] = (RxData[8] << 24) | (RxData[9] << 16)
							| (RxData[10] << 8) | RxData[11];
					uid_msg[2] = (RxData[12] << 24) | (RxData[13] << 16)
							| (RxData[14] << 8) | RxData[15];
					if (uid[0] != uid_msg[0] || uid[1] != uid_msg[1]
							|| uid[2] != uid_msg[2]) {
						/* If the jump address is valid AND the chip UID does not match then send NACK */
						OPENBL_FDCAN_SendByte(NACK_BYTE);
						return;
					}
				}
	...
	if (Common_GetProtectionStatus() != RESET) {
		...
		if ((data & 0xFFF0U) == 0xFFF0U) {
			if ((data == 0xFFFFU) || (data == 0xFFFEU) || (data == 0xFFFDU)) {
				error_value = OPENBL_MEM_MassErase(OPENBL_DEFAULT_MEM, RxData,
				FDCAN_RAM_BUFFER_SIZE);
			...
			error_value = OPENBL_MEM_Erase(OPENBL_DEFAULT_MEM, RxData,
			FDCAN_RAM_BUFFER_SIZE);
...
void OPENBL_FDCAN_WriteUnprotect(void) {
	...
	if (Common_GetProtectionStatus() != RESET) {
		...
		error_value = OPENBL_MEM_SetWriteProtection(DISABLE, OPENBL_DEFAULT_MEM,
		NULL, 0U);
...
uint8_t OPENBL_FDCAN_GetAddress(uint32_t *Address) {
	...
	*Address = (((((uint32_t) RxData[0]) << 24) | (((uint32_t) RxData[1]) << 16)
			| (((uint32_t) RxData[2]) << 8) | (((uint32_t) RxData[3]))));
...
void OPENBL_FDCAN_SpecialCommand(void) {
	...
	if (OPENBL_FDCAN_GetSpecialCmdOpCode(&op_code,
			OPENBL_SPECIAL_CMD) == NACK_BYTE) {
		OPENBL_FDCAN_SendByte(NACK_BYTE);
	} else {
		...
		if (special_cmd->SizeBuffer1 > SPECIAL_CMD_SIZE_BUFFER1) {
			...
			if (special_cmd->SizeBuffer1 != 0U) {
				if (special_cmd->SizeBuffer1 > 64U) {
					...
					OPENBL_FDCAN_ReadBytes(&RxData[64U],
							((uint32_t) special_cmd->SizeBuffer1) - 64U);
				} else {
					OPENBL_FDCAN_ReadBytes(RxData,
							((uint32_t) special_cmd->SizeBuffer1));
				}
...
void OPENBL_FDCAN_ExtendedSpecialCommand(void) {
	...
	/* Get the command operation code */
	if (OPENBL_FDCAN_GetSpecialCmdOpCode(&op_code,
			OPENBL_EXTENDED_SPECIAL_CMD) == NACK_BYTE) {
		OPENBL_FDCAN_SendByte(NACK_BYTE);
	} else {
		/* Send Operation code acknowledgment */
		OPENBL_FDCAN_SendByte(ACK_BYTE);
		...
		if (special_cmd->SizeBuffer1 > SPECIAL_CMD_SIZE_BUFFER1) {
			...
			if (special_cmd->SizeBuffer2 > SPECIAL_CMD_SIZE_BUFFER2) {
				...
				if (special_cmd->SizeBuffer2 != 0U) {
					...
					if (single != 0U) {
						OPENBL_FDCAN_ReadBytes(&RxData[data_length * 64U],
								single);
					}

					/* Read received bytes */
					for (index = 0U; index < special_cmd->SizeBuffer2;
							index++) {
