...
ErrorStatus OPENBL_MEM_RegisterMemory(OPENBL_MemoryTypeDef *Memory) {
	...
	if (NumberOfMemories < MEMORIES_SUPPORTED) {
		...
		a_MemoriesTable[NumberOfMemories].SetReadoutProtect =
				Memory->SetReadoutProtect;
		a_MemoriesTable[NumberOfMemories].SetWriteProtect =
...
uint32_t OPENBL_MEM_GetAddressArea(uint32_t Address) {
	...
	for (counter = 0U; counter < NumberOfMemories; counter++) {
		if ((Address >= a_MemoriesTable[counter].StartAddress)
				&& (Address < a_MemoriesTable[counter].EndAddress)) {
 ...
 */
ErrorStatus OPENBL_MEM_SetWriteProtection(FunctionalState State,
 ...
 */
ErrorStatus OPENBL_MEM_MassErase(uint32_t Address, uint8_t *p_Data,
		uint32_t DataLength) {
	uint32_t memory_index;
	ErrorStatus status;
	...
	if (memory_index < NumberOfMemories) {
		if (a_MemoriesTable[memory_index].MassErase != NULL) {
			status = a_MemoriesTable[memory_index].MassErase(p_Data,
					DataLength);
		} else {
			status = ERROR;
		}
 ...
 */
ErrorStatus OPENBL_MEM_Erase(uint32_t Address, uint8_t *p_Data,
		uint32_t DataLength) {
	uint32_t memory_index;
	ErrorStatus status;
