...
static FLASH_ProcessTypeDef FlashProcess = { .Lock = HAL_UNLOCKED, .ErrorCode =
...
static ErrorStatus OPENBL_FLASH_EnableWriteProtection(uint8_t *pListOfSectors,
		uint32_t Length);
static ErrorStatus OPENBL_FLASH_DisableWriteProtection(void);

/* Exported variables --------------------------------------------------------*/
OPENBL_MemoryTypeDef FLASH_Descriptor = { FLASH_START_ADDRESS,
FLASH_END_ADDRESS, FLASH_MEM_SIZE, FLASH_AREA, OPENBL_FLASH_Read,
		OPENBL_FLASH_Write, NULL, OPENBL_FLASH_SetWriteProtection,
		OPENBL_FLASH_JumpToAddress, NULL, OPENBL_FLASH_Erase };
...
void OPENBL_FLASH_Write(uint32_t Address, uint8_t *pData, uint32_t DataLength) {
	uint32_t index;
	__ALIGNED(4)
	uint8_t data[FLASH_PROG_STEP_SIZE] = { 0x0U };
	uint8_t remaining;
 ...
 */
ErrorStatus OPENBL_FLASH_SetWriteProtection(FunctionalState State,
		uint8_t *pListOfSectors, uint32_t Length) {
...
ErrorStatus OPENBL_FLASH_MassErase(uint8_t *pData, uint32_t DataLength) {
	...
	if (DataLength >= 2U) {
		...
		if (status == SUCCESS) {
			if (OPENBL_FLASH_ExtendedErase(&erase_init_struct, &sector_error)
					!= HAL_OK) {
				status = ERROR;
			} else {
				status = SUCCESS;
...
ErrorStatus OPENBL_FLASH_Erase(uint8_t *pData, uint32_t DataLength) {
	...
	for (counter = 0U;
			((counter < sectors_number) && (counter < (DataLength / 2U)));
			counter++) {
...
static ErrorStatus OPENBL_FLASH_EnableWriteProtection(uint8_t *pListOfSectors,
	...
	for (index = 0U; index < Length; index++) {
		if (*(pListOfSectors + index) < FLASH_SECTOR_31) {
	...
	for (index = 0U; index < Length; index++) {
		if (*(pListOfSectors + index) >= FLASH_SECTOR_31) {
			flash_ob.WRPSector |= (uint32_t) 1U
					<< ((*(pListOfSectors + index)) - (FLASH_SECTOR_31 + 1U));
...
#else
__attribute__((section(".ramfunc")))                     HAL_StatusTypeDef OPENBL_FLASH_WaitForLastOperation(
		uint32_t Timeout)
#endif /* (__ICCARM__) */
{
	uint32_t error;
...
#endif /* (__ICCARM__) */
{
	...
	status = OPENBL_FLASH_WaitForLastOperation(
			(uint32_t) OPENBL_FLASH_TIMEOUT_VALUE);
	...
	if (status == HAL_OK) {
		...
		*SectorError = 0xFFFFFFFFU;
		...
		if (((pEraseInit->Banks) & FLASH_BANK_2) == FLASH_BANK_2) {
			...
			pEraseInit->Sector = pEraseInit->Sector - (FLASH_SECTOR_31 + 1U);
		}

		/* Proceed to erase the sector */
		SET_BIT((*reg_cr),
				(FLASH_CR_SER | (pEraseInit->Sector << FLASH_CR_SNB_Pos) | FLASH_CR_START));
